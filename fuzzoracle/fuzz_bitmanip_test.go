...
func FuzzBitmanipVsLibriscv(f *testing.F) {
	type seed struct{ family uint8; a, b uint64 }
	seeds := []seed{
		// Zicsr
		{0, 0, 0x1F},          // CSRRW fcsr
		{1, 0, 0x7},           // CSRRS fcsr
	...
	f.Fuzz(func(t *testing.T, data []byte) {
		...
		case 2: // CSRRC x1, fcsr, x2
			...
			initRegs[2] = b & 0xFF
		// ── Zba ───────────────────────────────────────────────────────────
		case 3: insn = brt(0x10, 2, 1, 2, 3)  // SH1ADD
		case 4: insn = brt(0x10, 4, 1, 2, 3)  // SH2ADD
		case 5: insn = brt(0x10, 6, 1, 2, 3)  // SH3ADD
		case 6: insn = brt(0x04, 0, 1, 2, 3, 0x3B) // ADD.UW
		case 7: insn = brt(0x10, 2, 1, 2, 3, 0x3B) // SH1ADD.UW
		case 8: insn = brt(0x10, 4, 1, 2, 3, 0x3B) // SH2ADD.UW
		case 9: insn = brt(0x10, 6, 1, 2, 3, 0x3B) // SH3ADD.UW
		// ── Zbb logic ─────────────────────────────────────────────────────
		case 10: insn = brt(0x20, 7, 1, 2, 3)  // ANDN
		case 11: insn = brt(0x20, 6, 1, 2, 3)  // ORN
		case 12: insn = brt(0x20, 4, 1, 2, 3)  // XNOR
		case 13: insn = brt(0x05, 6, 1, 2, 3)  // MAX
		case 14: insn = brt(0x05, 7, 1, 2, 3)  // MAXU
		case 15: insn = brt(0x05, 4, 1, 2, 3)  // MIN
		case 16: insn = brt(0x05, 5, 1, 2, 3)  // MINU
		// ── Zbb rotate / sign-extend ──────────────────────────────────────
		case 17: insn = brt(0x30, 1, 1, 2, 3)  // ROL
		case 18: insn = brt(0x30, 5, 1, 2, 3)  // ROR
		case 19: insn = bimm(0x30, 5, 1, 2, int(b&63)) // RORI
		case 20: insn = brt(0x30, 1, 1, 2, 2)  // SEXT.B (funct7=0x30, rs2=2)
		case 21: insn = brt(0x30, 1, 1, 2, 3)  // SEXT.H (funct7=0x30, rs2=3)
		case 22: insn = brt(0x04, 4, 1, 2, 0)  // ZEXT.H
		// ── Zbs ───────────────────────────────────────────────────────────
		case 23: insn = brt(0x14, 1, 1, 2, 3)          // BSET
		...
		for r := uint8(1); r < 32; r++ { cpu.SetReg(r, initRegs[r]) }
		cpu.Step()
