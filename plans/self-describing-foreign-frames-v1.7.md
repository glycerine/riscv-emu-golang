...
2. [Stack Frame Layout](#2-stack-frame-layout)
3. [Magic+Version Word (RSP+8)](#3-magicversion-word-rsp8)
   - [3.1 Layout](#31-layout)
   - [3.2 Construction](#32-construction)
   - [3.3 Validation by the GC](#33-validation-by-the-gc)
   - [3.4 Safety Analysis](#34-safety-analysis)
4. [Header Word (RSP+16)](#4-header-word-rsp16)
...
5. [Cleanup Pointer (RSP+24)](#5-cleanup-pointer-rsp24)
   ...
   - [5.3 Calling Convention](#53-calling-convention)
   - [5.4 Cleanup Frame](#54-cleanup-frame)
   - [5.5 Cleanup Ownership](#55-cleanup-ownership)
   - [5.6 Cleanup Constraints](#56-cleanup-constraints)
   - [5.7 Runtime Integration](#57-runtime-integration)
6. [Preemption Protocol](#6-preemption-protocol)
   - [6.1 Problem](#61-problem)
   - [6.2 Solution](#62-solution)
   - [6.3 Foreign Code Contract](#63-foreign-code-contract)
   - [6.4 GC Cooperation](#64-gc-cooperation)
   - [6.5 What This Replaces](#65-what-this-replaces)
7. [Relationship to cgocall](#7-relationship-to-cgocall)
   - [7.1 What cgocall Provides](#71-what-cgocall-provides)
   - [7.2 Why cgocall Is Insufficient](#72-why-cgocall-is-insufficient)
   - [7.3 How This Protocol Differs](#73-how-this-protocol-differs)
   - [7.4 Symbolization](#74-symbolization)
   - [7.5 No cgo Toolchain Required](#75-no-cgo-toolchain-required)
8. [Tracked Region](#8-tracked-region)
   - [8.1 Location](#81-location)
   - [8.2 Bitmap Interpretation](#82-bitmap-interpretation)
   - [8.3 Maximal Bitmap Rule](#83-maximal-bitmap-rule)
...
9. [Untracked Region](#9-untracked-region)
   ...
   - [9.3 Permitted Uses](#93-permitted-uses)
10. [GC Scanner Integration](#10-gc-scanner-integration)
    - [10.1 Complete Scanner Logic](#101-complete-scanner-logic)
    - [10.2 Integration Point](#102-integration-point)
    - [10.3 Stack Traces](#103-stack-traces)
11. [JIT Compiler Requirements](#11-jit-compiler-requirements)
    - [11.1 At Compilation Time](#111-at-compilation-time)
    - [11.2 Emitted Prologue](#112-emitted-prologue)
    - [11.3 Emitted Epilogue](#113-emitted-epilogue)
    - [11.4 Emitted Callback Sequence](#114-emitted-callback-sequence)
    - [11.5 Emitted Cleanup Function](#115-emitted-cleanup-function)
    - [11.6 g Pointer Protocol](#116-g-pointer-protocol)
12. [Worked Example](#12-worked-example)
    - [12.1 Scenario](#121-scenario)
    - [12.2 Frame Design](#122-frame-design)
    - [12.3 Layout Computation](#123-layout-computation)
    - [12.4 Header Computation](#124-header-computation)
    - [12.5 Stack Layout](#125-stack-layout)
    - [12.6 Emitted Code](#126-emitted-code)
    - [12.7 GC Trace Walkthrough](#127-gc-trace-walkthrough)
    - [12.8 Panic Unwind Walkthrough](#128-panic-unwind-walkthrough)
13. [Constraints and Invariants](#13-constraints-and-invariants)
    - [13.1 JIT Compiler Invariants](#131-jit-compiler-invariants)
    - [13.2 Go Runtime Invariants](#132-go-runtime-invariants)
    - [13.3 Not Provided](#133-not-provided)
14. [Portability](#14-portability)
    - [14.1 ARM64](#141-arm64)
    - [14.2 Extensibility](#142-extensibility)
15. [Runtime Changes Summary](#15-runtime-changes-summary)
...
16. [Reference](#16-reference)
    ...
    - [16.3 Frame Layout Summary](#163-frame-layout-summary)
    - [16.4 Frame Size Formula](#164-frame-size-formula)
...
Arguments:
  ...
  arg1 (RSI on amd64, X1 on arm64):
    ...
    The cleanup may inspect this for
    logging or diagnostics. May be nil
    for runtime panics that carry no
    value.

No return value is used. The cleanup
returns normally and the unwind continues.
```
...
---
### 8.7 Departure from Go's Per-PC Liveness Model

Go's compiler emits PC-indexed pointer maps (PCDATA/FUNCDATA)
that vary the pointer/non-pointer interpretation of each stack
slot at each safepoint. This protocol uses a single
frame-lifetime bitmap instead. This section explains the
difference, why it is correctness-preserving, and what the
tradeoff is.

**Cases where Go's per-PC pointer maps are not just an
optimization but genuinely have different answers at different
PCs:**

- Union types in interface representations
  (the data word may be a pointer at one PC
  and an inlined integer at another).

- Stack slots reused by the register allocator
  for different variables across non-overlapping
  live ranges, where one variable was a pointer
  and another was a uintptr.

In Go's runtime, the same slot at the same offset truly
contains different categories of value at different PCs, and the
PCDATA index selects the correct interpretation at each
safepoint. The GC must use the PC-indexed map to trace correctly
without misinterpreting a non-pointer integer as a heap address.

**Why this protocol can use a single frame-lifetime bitmap
without breaking correctness.**

The maximal bitmap rule (Section 8.3) marks a slot as 1 if it
will EVER hold a Go pointer during the function's execution.
Combined with the zero-initialization requirement (Section 8.4)
and the slot lifecycle (Section 8.5), every slot marked 1
contains either nil or a real Go pointer when the GC observes
it.

When a slot transitions from holding a Go pointer to holding a
non-pointer value (the type-changing case above), the bitmap
still marks it 1. The GC traces the value. Two outcomes, both
benign:

- Value outside the Go heap range. The GC's
  inheap() check rejects it. No effect.

- Value coincidentally lies within the Go heap
  range. The GC pins the object at that
  address until the slot is overwritten or
  the frame returns. This is conservative
  retention, not a correctness violation.
  The pinned object is reachable as a root
  for a bounded duration: at most until the
  next observation where the slot has been
  overwritten with a different value, and
  always at most until the foreign frame
  returns and its bitmap ceases to exist.

**Why this is acceptable.**

Go's per-PC liveness is primarily a prompt-collection
optimization: it allows objects whose last use precedes a long
computation to be collected during that computation rather than
pinned to the end of the function. For a typical Go function
running thousands of safepoints across a long lifetime, the
optimization is significant.

For foreign frames in this protocol, three properties bound the
cost:

1. The GC observes the frame only at callback
   safepoints (Section 1.2, "No register maps
   needed"). Between callbacks, retention is
   invisible to the collector and irrelevant.

2. Foreign frames are typically short-lived
   relative to long-running Go functions. A
   JIT-compiled basic block, a translated
   guest function, or a hot-loop iteration
   returns quickly, and frame teardown
   destroys the bitmap.

3. Section 8.6 already provides the escape
   hatch: zero a tracked slot when its
   pointer is no longer needed. A JIT
   compiler that wants Go-equivalent
   precision can emit a clearing store at
   each "last use" point. The protocol does
   not require this, but supports it.

The retention introduced by a frame-lifetime bitmap is therefore
bounded by frame lifetime and explicitly under JIT-author
control via Section 8.6.

**Tradeoff summary.**

Foreign frames may retain Go objects past their last use within
the frame, up to frame return. JIT compilers concerned with
prompt collection should clear tracked slots at last-use points
per Section 8.6.
...
4. Cleanup reads context from [RDI+32],
   releases resources, returns normally.

5. callForeignCleanup returns to gopanic.
...
- Nested panic in cleanup (undefined).
