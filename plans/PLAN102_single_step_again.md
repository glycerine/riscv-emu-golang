...
## Bug fix: compilation hang on large blocks (ld_st test)

### Problem

With budget=1, `scanRegion` BFS still creates enormous regions (e.g., the ld_st test
scans ~2000 instructions, producing 167K-line dumps). The emitter compiles ALL of
them with per-instruction budget checks, creating thousands of budget cold-path
stubs. The assembler chokes on the massive Prog chain, causing a hang during
compilation at nStop=260.

With budget=1 we only ever execute 1 instruction per block — compiling 2000 is
pointless overhead.

### Fix

In `emitBlock` (`jit_emit_ir.go:941`), when lockstep mode is active, cap the
region end to limit compilation to a small number of instructions:
...
func (j *JIT) emitBlock(mem *GuestMemory, pc uint64) *emitResult {
    ...
    endPC := region.endPC
    if j.DebugOneBlockLockstepMode {
        // Limit region to budget+8 instructions (~40 bytes for RVC).
        // No point compiling thousands of instructions when budget=1.
        cap := pc + uint64((j.LockstepModeBudget+8)*4)
        if cap < endPC {
            endPC = cap
        }
    }
    return j.emitBlockRange(mem, pc, endPC)
}
```

This caps the region to roughly `budget+8` instructions (with 4 bytes per worst-case
instruction). For budget=1, the region is ~36 bytes — enough for the current
instruction plus a few for branch targets. Compilation stays fast.

### Files to modify

- `jit_emit_ir.go` — cap region in `emitBlock` when lockstepMode
