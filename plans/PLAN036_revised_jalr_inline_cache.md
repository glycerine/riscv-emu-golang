...
> **Status (implementation complete, but not enabled by default):**
> The 1-entry site IC is fully wired (IR op, lowering, backpatch,
> dispatcher branch, tests). When enabled on CoreMark/Dhrystone it
> **regresses MIPS by ~25-35%**. Root cause: polymorphic-return
> thrashing. `emitJALR` currently emits the baseline `WriteBackAll +
> RetDyn` sequence. Re-enabling requires Phase 1.5 (below) to add
> 2-way or patch-once-only logic.
>
> Measured:
>
> |workload|1-way IC MIPS|baseline MIPS|hit rate|misses|patches|
> |---|---|---|---|---|---|
> |bench_guest|3022|3343|n/a (few JALRs)|—|—|
> |coremark  |553 |750 |~51%|4.37 M|3.84 M|
> |dhrystone |347 |527 |~66%|3.63 M|3.13 M|
>
> Despite reducing Go round-trips by 6×, MIPS dropped. Root causes:
> 1. Every polymorphic miss re-patches (writes 16 bytes of the hot
>    code page), triggering self-modifying-code pipeline stalls on
>    each visit.
> 2. Inline CMPQ+JNE overhead runs on every JALR (hit and miss),
>    eating the savings on miss-majority sites.
>
> The IC infrastructure is retained behind the reverted `emitJALR`:
> a Phase 1.5 can enable a variant (patch-once, 2-way, or RAS) by
> flipping one line.
...
### Priority 1.5 — Fix the JALR IC polymorphism regression

The Step 5 flip in `emitJALR` was reverted. Infrastructure remains
wired. Four re-enable strategies, ranked by expected gain vs effort:

#### Option A — Patch-once (simplest)

Track `patched bool` per site in `jalrICPatchInfo`. First miss
patches; subsequent misses return to Go without re-patching. Still
pays the inline CMPQ+JNE on every JALR; miss path has no SMC penalty.

Expected: monomorphic sites (e.g., tail-recursive returns in a hot
loop) hit forever after one miss. Polymorphic sites settle into a
stable state (~1/N hit rate) with no thrashing. Should restore
near-baseline MIPS at worst; monomorphic-heavy workloads gain.

Effort: 5 lines in `tryPatchJalrIC`. Can be prototyped in an hour.

#### Option B — Thrash-detect deopt

Track `lastPatchedPC` and a bounce counter per site. If successive
misses report different targets N times, flip the site to "deopt":
leave cache_pc at an unmatchable sentinel, stop writing slots. Same
effect as Option A on polymorphic sites; more responsive to
near-monomorphic workloads that occasionally see an outlier.

Effort: 15 lines in `tryPatchJalrIC` + `jalrICPatchInfo`. Slightly
more nuanced but strictly better than A.

#### Option C — 2-way (2-slot) per-site IC

Double the inline sequence: two MOVABS+CMPQ pairs, two `cache_fn`
slots; miss path if neither matches. Updates the LRU slot on each
miss. Hit rate jumps dramatically on bi-modal sites (virtually all
function returns in structured code).

Cost: ~60-byte hit-path, vs 43 today. Extra ~3 cycles per hit.
Still within budget given eliminated round-trips.

Effort: 50 lines in `lowerJalrIC` + `emitJalrMissStub` + patch
logic. Moderate.

#### Option D — Return Address Stack (RAS)

For the specific case of `JALR rs1=x1, rd=x0` (RISC-V `ret`), use a
shadow stack rather than site-local IC. Each JAL with `rd=x1`
(`call`) pushes `pc+4`; each `ret` checks the stack top.

Hit rate on structured code: >95%. Doesn't help indirect calls /
virtual dispatch — those keep using the site IC.

Complications: tail calls (pop without push), setjmp/longjmp,
signal-like resumption. For pure compute benchmarks (Dhrystone,
CoreMark, bench_guest) none of those hit the hot path.

Effort: substantial (~200 lines). Separate shadow stack, push/pop
IR ops, lowering, overflow handling.

#### Recommendation

Ship Option A or B. Measure. If MIPS on CoreMark still below
target, escalate to C or D. Start with A (patch-once) — it's the
minimum change that protects against regression and gives some
monomorphic gain.
