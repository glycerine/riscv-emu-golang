...
func TestClearDirtySyscallRegs(t *testing.T) {
	...
	e.MarkDirty(VReg(11)) // a1
	e.MarkDirty(VReg(13)) // a3

	e.ClearDirtySyscallRegs()

	// a0 and a1 should be clean; others unchanged.
	if e.IsDirty(VReg(10)) {
		t.Error("a0 (x10) should be clean after ClearDirtySyscallRegs")
	}
	if e.IsDirty(VReg(11)) {
		t.Error("a1 (x11) should be clean after ClearDirtySyscallRegs")
	}
	if !e.IsDirty(VReg(5)) {
		t.Error("t0 (x5) should still be dirty")
	}
	if !e.IsDirty(VReg(13)) {
		t.Error("a3 (x13) should still be dirty")
	}
}
...
func TestReloadSyscallRegs(t *testing.T) {
	...
	// Should emit exactly 2 Load + 2 Zext = 4 instructions
	// (Load emits Load + Zext for sub-I64 types, but I64 loads
	//  don't get an extension — so 2 Loads only... actually
	//  Load with I64 and signed=false emits just the Load, no ext).
	// Actually: Load(vr, base, off, I64, false) emits 1 IRLoad.
	// So we expect 2 IRLoad instructions.
	loads := 0
	for i := before; i < after; i++ {
		if e.Block.Instrs[i].Op == IRLoad {
			loads++
		}
	}
	if loads != 2 {
		t.Errorf("ReloadSyscallRegs emitted %d loads, want 2", loads)
	}

	// Both a0 and a1 should be dirty after reload.
	if !e.IsDirty(VReg(10)) {
		t.Error("a0 (x10) should be dirty after ReloadSyscallRegs")
	}
	if !e.IsDirty(VReg(11)) {
		t.Error("a1 (x11) should be dirty after ReloadSyscallRegs")
	}

	// Verify the loads target x10 and x11 at the right offsets.
	loadIdx := 0
	expectedVR := []VReg{10, 11}
	expectedOff := []int64{80, 88} // 10*8, 11*8
	for i := before; i < after; i++ {
		ins := e.Block.Instrs[i]
		if ins.Op == IRLoad && loadIdx < 2 {
			if ins.Dst != expectedVR[loadIdx] {
				t.Errorf("load[%d] dst = %v, want x%d", loadIdx, ins.Dst, expectedVR[loadIdx])
			}
			if ins.A != e.XBase() {
				t.Errorf("load[%d] base = %v, want xBase", loadIdx, ins.A)
			}
			if ins.Imm != expectedOff[loadIdx] {
				t.Errorf("load[%d] offset = %d, want %d", loadIdx, ins.Imm, expectedOff[loadIdx])
			}
			loadIdx++
		}
	}
...
func TestReloadSyscallRegs_DoesNotAffectOtherRegs(t *testing.T) {
	...
	// x5 should still be dirty (untouched by reload).
	if !e.IsDirty(VReg(5)) {
		t.Error("x5 should still be dirty")
	}
	// x12 (a2) should NOT be dirty — reload only touches a0/a1.
	if e.IsDirty(VReg(12)) {
		t.Error("x12 should not be dirty — ReloadSyscallRegs only reloads a0/a1")
	}
}
