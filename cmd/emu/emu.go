...
func defineFlags(fs *flag.FlagSet, c *riscv.EmuConfig) {
	...
	fs.Var(riscv.SandboxMemoryModelFlag{Model: &c.MemoryModel}, "sandbox", "use sandboxed guest memory for -run; default linear JIT skips the sandbox mask and jea9linux access checks")
	...
	fs.Uint64Var(&c.Breadcrumb.StopAt, "breadcrumb-stop", 0, "stop hashing and raise the breadcrumb trip IRQ at this breadcrumb sequence number")
